...
uint32_t printPacketEditTmpDsMemMac(char *fmtStr, void *data) {
    ...
    sprintf(fmtStr, "PacketEditTmpDsMemMac : \n}\n");
    return strlen(fmtStr);
}

int32_t  readPacketEditTmpDsMemMac(tMemRegHdr *memPtr, uint32_t index, void *output) {
    assert(output != NULL);
    if(index >= memPtr->entryNum.entries) {
        sprintf(output, "ERROR: out of range PacketEditTmpDsMemMac, max %" PRIu64 " but %d\n", memPtr->entryNum.entries-1, index);
        return -1;
    }
    ...
    for (i = 0; i < nflds; i++) {
        ...
        if (num != 2) {
            servLogError("Illegal assignment expression [%s] in fldSet function.\r\n", *(flds + i));
                ...
                servLogWarn("Unknown field %s in PacketEditTmpDsMemMac.\r\n", *(flds + i));
            }
        }
    }
    return wrFlag;
...
uint32_t fldGetPacketEditTmpDsMemMac(tMemRegHdr *memPtr, uint32_t index, char *fldName, void *output, uint8_t fromWhere) {
    if(index >= memPtr->entryNum.entries) {
        sprintf(output, "ERROR: out of range PacketEditTmpDsMemMac, max %" PRIu64 " but %d\n", memPtr->entryNum.entries-1, index);
        return -1;
    }
...
int32_t  writePacketEditTmpDsMemMac(tMemRegHdr *memPtr, uint32_t index, uint16_t parac, char *parav[], void *output, uint8_t fromWhere) {
    if(index >= memPtr->entryNum.entries) {
        sprintf(output, "Error: Out of range PacketEditTmpDsMemMac, max %" PRIu64 " but %d in write function.\n", memPtr->entryNum.entries-1, index);
        return -1;
    }
    ...
    readHwEntry(ptr, memPtr, index, memPtr->widthInQuad, fromWhere);
#endif
    int modified = fldSetPacketEditTmpDsMemMac(ptr, parac, parav);
#if NEED_ADDR == 1
    if (modified == 1) {
...
int32_t setPacketEditTmpDsMemMac(tMemRegHdr *memPtr, uint32_t index, void *data, void *output, uint8_t fromWhere) {
    assert(data != NULL);
    if(index >= memPtr->entryNum.entries) { 
        sprintf(output, "Error: Out of range PacketEditTmpDsMemMac, max %" PRIu64 " but %d in set function\n", memPtr->entryNum.entries-1, index);
        return -1;
    }
    pthread_rwlock_wrlock(memPtr->rwLockList + index);
...
int32_t resetPacketEditTmpDsMemMac(tMemRegHdr *memPtr, uint8_t chgData) {
    if(chgData) {
