...
#define _log_debug(lhs, format, ...) write_log(lhs, LOG_DEBUG, format, __VA_ARGS__)
#define _log_info(lhs, format, ...)  write_log(lhs, LOG_INFO, format, __VA_ARGS__)
#define _log_warn(lhs, format, ...)  write_log(lhs, LOG_WARN, format, __VA_ARGS__)
#define _log_error(lhs, format, ...) write_log(lhs, LOG_ERROR, format, __VA_ARGS__)

#define write_log(lhs, level, format,  ...) _log_write(lhs, level, format, __FILE__, _STR(__LINE__), __VA_ARGS__)
...
typedef enum log_level_t {
    _DEBUG_LEVEL = LOG_DEBUG,    ///< debug
    _INFO_LEVEL = LOG_INFO,    ///< info
    _WARN_LEVEL = LOG_WARN,    ///< warning
    _ERROR_LEVEL = LOG_ERROR,    ///< error
} log_level_t;
 ...
 */
log_handle_t* file_handle_create(const char* log_filename, size_t max_file_size, size_t max_file_bak, size_t max_iobuf_size, uint8_t cflag, const char* password, uint8_t level);
...
/** log with timestamp if flag sets 1 */
