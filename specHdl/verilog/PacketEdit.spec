...
process PacketEdit(PI, glPktNew, MsgAdmInfo) {
    ...
    if( PI.destPort == CtlCommon.cpuPortNo[0:0] ) {
        PI.forbidEdit    |= CtlEgrCpu.forbidEdit[0:0];
        PI.addCpuHdr      = CtlEgrCpu.needCpuHdr[0:0] && PI.firstDecideToCpu;
        PI.forceDest      = 1;
    ...
    if( !PI.forceDest && (PI.destPort != CtlCommon.cpuPortNo[0:0]) ) {
    ...
    if( PI.ptpValid ) {
        ...
        if( PI.l4Type == L4_TYPE_UDP && PI.l3Type == L3_TYPE_IPV4 && CtlEgrEdit.v4PtpUdpChksumSet0[0:0] ) {    // UDP checksum is 0
    ...
    PI.updateMacSa = !PI.forbidEdit && (PI.routingEn || (PI.ptpValid && CtlEgrEdit.ptpUpdMacSaEn[0:0] && PI.ptpEn));
    ...
    if( PI.ptpValid && CtlEgrEdit.ptpUpdMacSaEn[0:0] && PI.ptpEn ) {
    ...
    PI.tsOpEn         = 0;
    PI.updatePtpChksum = (PI.l4Type == L4_TYPE_UDP && (!CtlEgrEdit.v4PtpUdpChksumSet0[0:0] || CtlEgrEdit.updV6PtpUdpChksum[0:0])) || PI.updPtpChksum;
    if (PI.ptpValid && PI.ptpEn && 
        ( ((PI.ptpType == PTP_MSG_PDELAY_RESP) && (PI.tsOp == PTP_OP_ONE_STEP)) || 
          ((CtlEgrEdit.ptpDevType[0:0][0:0] == PTP_DEV_TC_P2P || CtlEgrEdit.ptpDevType[0:0][0:0] == PTP_DEV_TC_E2E) && (PI.tsOp == PTP_OP_ONE_STEP) && (PI.ptpType == PTP_MSG_SYNC)) || 
          ((CtlEgrEdit.ptpDevType[0:0] == PTP_DEV_TC_E2E) && (PI.ptpType == PTP_MSG_DELAY_REQ)) || 
          ((CtlEgrEdit.cfMissedType[15:0] & (1 << PI.ptpType)) != 0 ) 
        )
       ) {        // Update Correction Field 
        PI.tsOpEn              = 1;
        if (PI.ptpType == PTP_MSG_SYNC && PI.tsOp == PTP_OP_ONE_STEP && CtlEgrEdit.ptpDevType[0:0] == PTP_DEV_TC_P2P) {
        ...
        PI.updPtpCf     = 1;
    } else if (PI.ptpValid && PI.ptpEn && ((CtlEgrEdit.ptpDevType[0:0][0:0] == PTP_DEV_BC || CtlEgrEdit.ptpDevType[0:0][0:0] == PTP_DEV_OC) && 
        ...
        PI.updPtpTs     = 1;
               ...
               ( ((PI.ptpType == PTP_MSG_SYNC) && (PI.tsOp == PTP_OP_TWO_STEP)) || (PI.ptpType == PTP_MSG_PDELAY_REQ) ||
                 ((PI.ptpType == PTP_MSG_DELAY_REQ) && (CtlEgrEdit.ptpDevType[0:0][0:0] == PTP_DEV_BC || CtlEgrEdit.ptpDevType[0:0][0:0] == PTP_DEV_OC)) ||
    ...
    CtlEgrLog.egrDiscard[0:0] = PI.egrDiscard;
    CtlEgrLog.routedPkt[0:0] = PI.routedPkt;
    CtlEgrLog.removeCpuHdr[0:0] = PI.removeCpuHdr;
    CtlEgrLog.addCpuHdr[0:0] = PI.addCpuHdr;
    CtlEgrLog.updateMacDa[0:0] = PI.updateMacDa;
    CtlEgrLog.updateMacSa[0:0] = PI.updateMacSa;
    CtlEgrLog.removeVlanTag[0:0] = PI.removeVlanTag;
    CtlEgrLog.addVlanTag[0:0] = PI.addVlanTag;
    CtlEgrLog.remoteSpan[0:0] = PI.remoteSpan;
    CtlEgrLog.addRtag[0:0] = PI.addRtag;
    CtlEgrLog.replaceCos[0:0] = PI.replaceCos;
    CtlEgrLog.replaceDscp[0:0] = PI.replaceDscp;
    CtlEgrLog.egrStpChkFail[0:0] = PI.egrStpChkFail;
    CtlEgrLog.ptpPortTypeChkFail[0:0] = PI.ptpPortTypeChkFail;
    CtlEgrLog.ptpType[15:0] = PI.ptpType[15:0];
    CtlEgrLog.updPtpTs[0:0] = PI.updPtpTs;
    CtlEgrLog.updPtpCf[0:0] = PI.updPtpCf;
    CtlEgrLog.intrPtpTs[0:0] = PI.intrPtpTs;
    CtlEgrLog.xconErr[0:0] = PI.xconErr;
    CtlEgrLog.truncateCpuPkt[0:0] = PI.truncateCpuPkt;
}
