...
function DestPost(PI, MsgIgr2Adm) {
    ...
    if( ((PI.vlanId < CtlFwdPortIsolate.vlanId[0:0]) || CtlFwdPortIsolate.forcePortEgressMask[0:0])
        && !PI.routingEn ) {
    ...
    // Intial FwdBitmap
    tmpVlanPortMask = DsMemVlanPortMaskVal.portMask | (~CtlFwdEgressFilter.egrFilteringEn[0:0]);
    ...
    if( PI.opCode == OP_CODE_UCAST || PI.opCode == OP_CODE_NAT ) {
        ...
        if( DsMemMcastPortVal.applyVlanMask ) {
            tmpFwdingBmp     = ( (~CtlFwdMcast.mcFilteringEn[0:0] | tmpVlanPortMask) & tmpFwdingBmp );
        }
        if( CtlFwdMcast.mcastExcludeSrc[0:0] ) 
        ...
        DsRegLagGrp26Val    = DsRegLagGrp[ 26 ];      // read LAG group table
        ...
        (tmpIsUcastLinkAgg && (30 == tmpDestLinkAggId)) ) && DsRegLagGrp30Val.linkNumber > 0 ) {
            ...
            tmpLagFwdBmp30[ tmpLagDestPort:tmpLagDestPort ] = 1;
        }
    ...
    if( (PI.opCode == OP_CODE_BCAST) && !PI.routingEn ) {
        if( PI.isBcastMac ) {
            PI.fwdBitmap = PI.fwdBitmap & (~CtlFwdUnknownFilter.bcastFilter[0:0]);
        } else if( PI.isMcastMac ) {
            PI.fwdBitmap = PI.fwdBitmap & (~CtlFwdUnknownFilter.unknownMcastFilter[0:0]);
            PI.mcastFlood = 1;
        } else {
            PI.fwdBitmap = PI.fwdBitmap & (~CtlFwdUnknownFilter.unknownUcastFilter[0:0]);
            PI.ucastFlood = 1;
        }
    }
    // To CPU packet
    tmpExcpToCpuFlag    = PI.exception && (CtlFwdExcp.excepToCpu[0:0] & (1 << PI.excpType));
    ...
    if(PI.ptpValid && PI.srcPort != CtlCommon.cpuPortNo[0:0] && PI.srcPort != 6 ) {
        if( (CtlFwdDest.redirPtpType[0:0] & (1 << PI.ptpType)) != 0 ) {
            PI.fwdBitmap     = 0;
            PI.redirPtp      = 1;
        }
        if((CtlFwdDest.ptpSnifferType[0:0] & (1 << PI.ptpType)) != 0) {
            PI.fwdBitmap[CtlCommon.cpuPortNo:CtlCommon.cpuPortNo] = 1;
            PI.criticalPacket    = 1;
            PI.firstDecideToCpu  = 1;
            PI.ptp2Cpu           = 1;
    ...
    tmpEgrSpanFlag[0:0] = (PI.fwdBitmap & CtlFwdEgressSpan.l2EgrSpanEn) != 0;
    PI.fwdBitmapW1[1:1]        = tmpEgrSpanFlag;
    ...
    tmpBitmap           = 0;
        ...
        if( PI.fwdBitmap[0:0] ) {
            ...
            tmpBitmap[ tmpChnlId:tmpChnlId ] = 1;
        }
    ...
    CtlFwdLog.fwdBitmap[2:0] = PI.fwdBitmap[2:0];
    CtlFwdLog.fwdBitmapW1[2:0] = PI.fwdBitmapW1[2:0];
    CtlFwdLog.vlanDiscard[0:0] = PI.vlanDiscard;
    CtlFwdLog.mcastFlood[0:0] = PI.mcastFlood;
    CtlFwdLog.destMap[10:0] = PI.destMap[10:0];
    CtlFwdLog.opCode[2:0] = PI.opCode[2:0];
    CtlFwdLog.ucastFlood[0:0] = PI.ucastFlood;
    CtlFwdLog.criticalPacket[0:0] = PI.criticalPacket;
    CtlFwdLog.forbidEdit[0:0] = PI.forbidEdit;
    CtlFwdLog.redirPtp[0:0] = PI.redirPtp;
    CtlFwdLog.egrPortFilted[0:0] = PI.egrPortFilted;
    CtlFwdLog.egrVlanFilted[0:0] = PI.egrVlanFilted;
    CtlFwdLog.lagFilted[0:0] = PI.lagFilted;
    CtlFwdLog.ptpValid[0:0] = PI.ptpValid;
    CtlFwdLog.ptp2Cpu[0:0] = PI.ptp2Cpu;
    CtlFwdLog.addRtag[0:0] = PI.addRtag;
    CtlFwdLog.tsnGateId[4:0] = PI.tsnGateId[4:0];
    CtlFwdLog.tsnCycle[2:0] = PI.tsnCycle[2:0];
    CtlFwdLog.cpuPktType[3:0] = PI.cpuPktType[3:0];
    CtlFwdLog.destLostInCfg[0:0] = PI.destLostInCfg;
